...
$('#add').click(function () {
    ...
    var forcomplete = {};
    var object = 6;
    var documentname = "Purchase Invoice";
    $.ajax({
        type: "GET",
        ...
        complete: function () {
            if (!forcomplete) {
                ...
                $('#ModalTitle').html('Create Purchase Invoice');
                $('#purchaseinvoice').text('Add Purchase Invoice').removeClass('btn btn-success').addClass('btn btn-primary');
                clearAll();
                LoadSeries();
    ...
    $('#tableListItem tbody tr').each(function (index, ele) {
        if ($("#itemcode", this).val() != "" && $("#itemname", this).val() != "") {
            var item = {
                ...
                WTax: Number($('#linewtax-' + index).val().replace(/,/g, '')),
...
$('#series').change(function () {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        url: requestURL,
        data: { series: series, objectcode: 6 },
        dataType: "json",
        success: function (data) {
            forcomplete = data;
        },
                                ...
                                complete: function () {
                                    ...
                                    $.each(forcomplete.Lines, function (i, item) {
                                        ...
                                        $item.append(
                                            ...
                                            '<td><select id="itemcode-' + ItemRow + '" class="form-control chosen-select" onchange="ItemCodeOnChange(getRowID(this))"></select></td>' +
                                            '<td><select id="itemname-' + ItemRow + '" class="form-control chosen-select" onchange="ItemNameOnChange(getRowID(this))"></select></td>' +
                                            '<td> <input type="text" id="qty-' + ItemRow + '" value="' + addThousandsSeparator(item.Quantity.toFixed(2)) + '" class="c4 form-control text-right" placeholder="0.00" onchange="QtyOnChange(getRowID(this));updatetotal();"  onkeypress="return isNumberKey(event)" /> </td>' +
                                            '<td><input type="text" class="form-control" id="uom-' + ItemRow + '" value="PIECE" disabled /></td>' +
                                            '<td> <input type="text" id="linediscount-' + ItemRow + '" value="' + addThousandsSeparator(item.Discount.toFixed(2)) + '" class="c4 form-control text-right" placeholder="0.00" onchange="calculate_row(getRowID(this));updatetotal();"  onkeypress="return isNumberKey(event)" /></td>' +
                                            ...
                                            '<td> <input type="hidden" id="pricelistgross-' + ItemRow + '"> <input type="text" id="linegross-' + ItemRow + '" value="' + addThousandsSeparator(item.GrossPrice.toFixed(2)) + '" class="c4 form-control text-right" placeholder="0.00" onchange="calculate_row(getRowID(this),true);updatetotal();" onkeypress="return isNumberKey(event)" /></td>' +
...
function AddNewItemRow() {
    ...
    if (isValid) {
        ...
        $item.append(
            ...
            '<td><select id="itemcode-' + ItemRow + '" class="form-control chosen-select" onchange="ItemCodeOnChange(getRowID(this))"></select></td>' +
            '<td><select id="itemname-' + ItemRow + '" class="form-control chosen-select" onchange="ItemNameOnChange(getRowID(this))"></select></td>' +
            '<td> <input type="text" id="qty-' + ItemRow + '" class="c4 form-control text-right" placeholder="0.00" onchange="QtyOnChange(getRowID(this));updatetotal();"  onkeypress="return isNumberKey(event)" /> </td>' +
            //'<td><input type="text" class="form-control" id="uom-' + ItemRow + '" value="PIECE" disabled /></td>' +
            '<td><select id="uom-' + ItemRow + '" class="form-control chosen-select"></select></td>' +
            '<td> <input type="text" id="linediscount-' + ItemRow + '" class="c4 form-control text-right" placeholder="0.00" onchange="calculate_row(getRowID(this));updatetotal();"  onkeypress="return isNumberKey(event)" /></td>' +
            '<td><input type = "hidden" id = "linetotal-' + ItemRow + '" class= "c4 form-control text-right" />' +
            ...
            '<td><select id="whse-' + ItemRow + '" class="form-control chosen-select"></select></td>' +
            '<td><select id="linevat-' + ItemRow + '" class="form-control chosen-select" onchange="calculate_row(getRowID(this));updatetotal();"></select></td>' +
            '<td> <input type="hidden" id="pricelistgross-' + ItemRow + '"> <input type="text" id="linegross-' + ItemRow + '" class="c4 form-control text-right" placeholder="0.00" onchange="calculate_row(getRowID(this),true);updatetotal();" onkeypress="return isNumberKey(event)" /></td>' +
        ...
        PoppulateVat(ItemRow);
        PoppulateUoM(ItemRow,"PIECE");
        ItemRow++;
    }
}
...
function PoppulateUoM(row, uom) {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            if (uom != null) {
                $('#uom-' + row).val(uom).trigger("chosen:updated");
            }
            $('.chosen-select').chosen({ width: "100%" });
        }
    });
...
function calculate_row(row, inputgross) {
    var qty = parseFloat($('#qty-' + row).val().replace(',', ''));
    //var unitprice = parseFloat($('#unitprice', $this).val().replace(',', ''));
    var linediscount = parseFloat($('#linediscount-' + row).val().replace(',', ''));
    //var priceafdiscount = parseFloat($('#priceafdiscount', $this).val().replace(',', ''));
    //var linetotal = parseFloat($('#linetotal-'+ $this).val().replace(',', ''));
    var linewtax = parseFloat($('#linewtax-' + row).val().replace(',', ''));
    var linevat = parseFloat($('#linevat-' + row + ' option:selected').val().replace(',', ''));
    var linegross = 0;
    if (inputgross === true) {
        linegross = parseFloat($('#linegross-' + row).val().replace(',', ''));
        $('#pricelistgross-' + row).val(addThousandsSeparator(linegross.toFixed(2)));
    }
    else {
        linegross = parseFloat($('#pricelistgross-' + row).val().replace(',', ''));
    }
    var linegrosstotal = parseFloat($('#linegrosstotal-' + row).val().replace(',', ''));
    ...
    var newgrossprice = (linegross * (linevat + 1)) * ((100 - linediscount) / 100);
...
function updatetotal(discounttype) {
    ...
    $('#tableListItem tbody tr').each(function (i, item) {
        ...
        if (isNaN(linevat)) { linevat = 0; }
        total += parseFloat($('#linegrosstotal-' + i).val().replace(/,/g, ''));
        wtax += parseFloat($('#linegrosstotal-' + i).val().replace(/,/g, '')) * (parseFloat($('#linewtax-' + i).val().replace(/,/g, '')) / 100);
        //vat += parseFloat($('#linegrosstotal-' + i, this).val().replace(/,/g, '')) * (parseFloat(linevat) / 100);
        vat += parseFloat($('#linegrosstotal-' + i).val().replace(/,/g, '')) * (parseFloat(linevat) + 1);
    });
    discount = parseFloat($('#discount').val().replace(/,/g, ''));
    discountamount = parseFloat($('#discountamount').val().replace(/,/g, ''));
